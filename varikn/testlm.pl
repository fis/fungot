...
# Slurp in and parse the model file.
...
sub read_node
{
    ...
    {
	$childs->{$cr->[0]} = read_node($file, $cr->[1]);
    }
...
$in = undef;
open $in, '<:raw', $modelfile or die "can't read: $modelfile: $!";
my $tree = read_node($in, 0);
close $in;
...
sub descend
{
    ...
    return descend($child, $context, $at-1);
}
