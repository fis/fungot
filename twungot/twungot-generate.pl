...
# twungot tweet-generation code.
# based on testlm-disk.pl.
...
scalar @ARGV >= 2 or die "usage $0 tokens.bin.foo model.bin.foo [length] [count] [prefix ...]";
my ($tokfile, $modelfile, $len, $count, @prefix) = @ARGV;
$len ||= 125;
$count ||= 1;
